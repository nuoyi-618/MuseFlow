...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 21:01:22 2020
...
def write_midi(filepath, pianorolls, program_nums=None, is_drums=None,
    ...
    multitrack = Multitrack(tempo=tempo)
...
def save_midi(filepath, phrases, config):
    ...
    pad_width = ((0, 0),
                 (0, config['pause_between_samples']),
                 (config['lowest_pitch'],
    ...
    write_midi(filepath, pianorolls, config['programs'], config['is_drums'],config['track_names'], tempo=config['tempo'], beat_resolution=config['beat_resolution'])
