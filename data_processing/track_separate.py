...
def cal_file_features(midi_file):
    ...
    #pm=pretty_midi pypiano=pypianoroll
    pm, pypiano = remove_empty_track(midi_file)
    ...
    rel_durations = relative_duration(pypiano)#持续时间
    number_notes = number_of_notes(pm) #音符数
    occup_rate = occupation_rate(pypiano) #占有率
    poly_rate = polyphony_rate(pypiano) #复调率
...
if __name__ == "__main__":
    ...
    if len(args.file_name) > 0:
        ...
        #all_names = get_file_path(args.input_folder)
